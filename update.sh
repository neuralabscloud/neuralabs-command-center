  ...
  for component in command-center config; do
    if [ -d "$SCRIPT_DIR/$component" ]; then
      rsync -a --delete \
        ...
        --exclude='data/api-usage.json' \
        --exclude='data/api-costs-config.json' \
        ...
        --exclude='data/ugc-audio/' \
        --exclude='data/infsh-input-*.json' \
        --exclude='data/ai-vid-input-*.json' \
